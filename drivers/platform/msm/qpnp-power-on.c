...
#ifdef CONFIG_AL_PLATFORM
		if (cfg->pon_type == PON_CBLPWR) {
			input_report_switch(pon->pon_input, cfg->key_code, 1);
		} else {
...
#ifdef CONFIG_AL_PLATFORM
		if (cfg->pon_type == PON_CBLPWR) {
			input_report_switch(pon->pon_input, cfg->key_code, key_status);
		} else {
...
#ifdef CONFIG_AL_PLATFORM
	if (cfg->pon_type == PON_CBLPWR) {
		input_set_capability(pon->pon_input, EV_SW, cfg->key_code);
	} else {
