/* 2015-11-26: File added and changed by Sony Corporation */
/*
 *
...
char *taglist[] = {
	...
	"build_product",
	"build_variant",
	"pmic_PM8994_revision_str",
	"pmic_PMI8994_revision_str",
	"pmic_PMI8996_revision_str",
	"socinfo_build_id",
	"socinfo_version",
	"socinfo_id",
	"socinfo_raw_id",
	"socinfo_platform_type",
	"socinfo_platform_version",
	"rdinfo_processname",
	"rdinfo_type",
	"rdinfo_log",
	"rdinfo_db_tag",
...
#ifdef CONFIG_AL0_RAMDUMP
		dev_dbg(dev, "Could not get data for tag \"%s\": %d!\n",
			name, ret);
#else
...
#ifdef CONFIG_AL0_RAMDUMP
/*store format rdtag_<tagname>: <tagdata> */
#define RDTAG_FORMAT 20

static ssize_t taglist_read(char __user *ubuf, size_t len, loff_t *offset)
{
	size_t count = 0, bufsize, fmt_len, written = 0;
	static unsigned int tag;
	unsigned int tagsize = 0, num_of_tags;
	char *buf;

	num_of_tags = sizeof(taglist)/sizeof(taglist[0]);
	...
	while (tag < num_of_tags) {

		if (rdtags_get_tag_data(taglist[tag], NULL,
					&bufsize) != -ENOBUFS) {
			tag++;
			continue;
		}

		fmt_len = (sizeof(taglist[tag]) + RDTAG_FORMAT + bufsize);
		buf = kzalloc(fmt_len, GFP_KERNEL);
		if (!buf) {
		...
		written = snprintf(buf, (sizeof(taglist[tag]) + RDTAG_FORMAT +
					6), "rdtag_%s: ", taglist[tag]);
		if (rdtags_get_tag_data(taglist[tag], (buf + written),
					&bufsize)) {
			dev_err(dev, "Could not find tag \"%s\"!\n",
				taglist[tag] ? taglist[tag] : "NULL");
			kfree(buf);
			buf = NULL;
			tag++;
			continue;
		...
		if (copy_to_user(ubuf + count, buf , written))
			return -EFAULT;
...
#ifdef CONFIG_AL0_RAMDUMP
	if (strncmp(name, "taglist", 7) == 0) {
		count = taglist_read(ubuf, len, offset);
		goto exit;
	}
#endif
...
#ifdef CONFIG_AL0_RAMDUMP
	if (procfs_create_node("taglist") < 0)
		dev_warn(dev, "Failed to create proc subentry - taglist\n");
#endif
...
static void __exit rdtags_module_exit(void)
{
	...
	if (entry) {
#ifdef CONFIG_AL0_RAMDUMP
		remove_proc_entry("event_history", entry);
#endif
