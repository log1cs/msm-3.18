/* 2015-10-22: File added and changed by Sony Corporation */
/*
...
#ifdef CONFIG_AL0_RAMDUMP
int ramdump_reset_mem_desc(void)
{
	struct ramdump_mem_desc *m_desc = NULL;

	if (!mem_desc_base) {
		dev_info(dev, "Invalid memory descriptors base\n");
		return -EINVAL;
	}

	m_desc = (struct ramdump_mem_desc *)mem_desc_base;
	m_desc->hdr.num_desc = 0;

	memset_io(mem_desc_base + sizeof(struct mem_desc_hdr),
		0x0, mem_desc_size - sizeof(struct mem_desc_hdr));
	pr_info("memory descriptors area set to zero\n");

	return 0;
}
EXPORT_SYMBOL(ramdump_reset_mem_desc);
#endif
