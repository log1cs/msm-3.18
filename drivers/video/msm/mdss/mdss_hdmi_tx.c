...
static void hdmi_tx_hpd_off(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	rc = hdmi_tx_enable_power(hdmi_ctrl, HDMI_TX_CEC_PM, 0);
	if (rc)
		DEV_INFO("%s: Failed to disable cec power. Error=%d\n",
			__func__, rc);
#endif
...
static int hdmi_tx_hpd_on(struct hdmi_tx_ctrl *hdmi_ctrl)
{
	...
	if (hdmi_ctrl->hpd_initialized) {
		...
		if (rc) {
			...
			DEV_ERR("%s: cec enable power failed rc = %d\n",
				__func__, rc);
			return rc;
		}
#endif
