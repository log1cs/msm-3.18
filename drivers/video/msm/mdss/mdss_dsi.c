...
#ifdef CONFIG_SOMC_TONE
static void set_touch_power(bool enable, struct mdss_panel_data *pdata)
{
	int ret;
	...
	if(enable){
		...
		if (ret) {
			pr_warn("%s: Set touch vreg failed. rc=%d\n", __func__, ret);
			ret = 0;
		}

		if (gpio_is_valid(ctrl_pdata->touch_vddio_gpio))
			gpio_direction_output(ctrl_pdata->touch_vddio_gpio, 0);
		...
		if (gpio_is_valid(ctrl_pdata->touch_vddio_gpio))
			gpio_set_value(ctrl_pdata->touch_vddio_gpio, 1);
...
#ifdef CONFIG_SOMC_TONE
	ctrl_pdata->touch_vddio_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
