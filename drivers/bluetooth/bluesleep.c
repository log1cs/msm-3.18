/* 2017-07-13: File changed by Sony Corporation */
/*
...
/* 0.5 second timeout */
#define BT_UART_TIMER_INTERVAL  500
/** UART timer */
static void bluesleep_uart_timer_expire(unsigned long data);
static DEFINE_TIMER(uart_timer, bluesleep_uart_timer_expire, 0, 0);
...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
	...
	} else if (test_bit(BT_ASLEEP, &flags) && !test_bit(BT_SUSPEND, &flags)) {
		...
		if (debug_mask & DEBUG_VERBOSE)
			pr_err("start uart timer\n");
		mod_timer(&uart_timer, (jiffies + BT_UART_TIMER_INTERVAL));
...
static void bluesleep_uart_timer_expire(unsigned long data)
{
	...
	if (debug_mask & DEBUG_VERBOSE)
		pr_info("uart timer expired\n");

	spin_lock_irqsave(&rw_lock, irq_flags);
	set_bit(BT_TIMEOUT, &flags);
...
static int __init bluesleep_init(void)
{
	...
	uart_timer.function = bluesleep_uart_timer_expire;
	uart_timer.data = 0;
