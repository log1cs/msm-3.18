/* 2017-08-04: File changed by Sony Corporation */
/*
...
void wifi_card_detect(bool on)
{
	...
	if (on) {
		wifi_mmc_host->rescan_entered = 0;
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (msm_host->pdata->nonremovable) {
		...
		if (of_property_read_bool(pdev->dev.of_node, "cd-post")) {
			msm_host->mmc->caps2 |= MMC_CAP2_CD_POST;
		}
	}
