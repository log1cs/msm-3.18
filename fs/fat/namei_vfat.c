...
#define	GET_INODE_USER_ID(x)	((x)->i_uid.val ? 1 : 0)
...
int get_vfat_posix_attr(struct inode *inode, struct msdos_dir_entry *dentry)
{
	...
	/* User & Group ID */
	px_uid = sbi->options.fs_uid.val ? sbi->options.fs_uid.val :
		 CONFIG_SNSC_FS_VFAT_POSIX_ATTR_DEFAULT_UID_VALUE;
	px_gid = sbi->options.fs_gid.val ? sbi->options.fs_gid.val :
		 CONFIG_SNSC_FS_VFAT_POSIX_ATTR_DEFAULT_UID_VALUE;
	inode->i_uid.val = get_pxattr_uid(dentry) ?  px_uid : 0;
	inode->i_gid.val = get_pxattr_uid(dentry) ?  px_gid : 0;
...
#ifdef CONFIG_SNSC_FS_VFAT_COMPARE_UNICODE
static int vfat_cmpiu(const struct dentry *parent, const struct dentry *dentry,
		unsigned int len, const char *str, const struct qstr *name)
{
	struct nls_table *t = MSDOS_SB(parent->d_sb)->nls_io;
...
#ifdef CONFIG_SNSC_FS_VFAT_COMPARE_UNICODE
static int vfat_cmpu(const struct dentry *parent, const struct dentry *dentry,
		unsigned int len, const char *str, const struct qstr *name)
{
	struct nls_table *t = MSDOS_SB(parent->d_sb)->nls_io;
	int alen, blen, aulen, aunilen, bulen, bunilen;
