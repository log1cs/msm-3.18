/* 2017-11-30: File changed by Sony Corporation */
	     ...
	     int perf_mode, uint16_t bit_width, int app_type, int acdb_id)
{
	...
	if ((topology == VPM_TX_SM_ECNS_COPP_TOPOLOGY) ||
	    ...
	    (topology == CAPI_V2_LIBMFEBUNDLE_TX_TOPOLOGY) ||
#endif
	...
	struct libmfe_sound_direction_param *soundDirectionData)
{
	...
	char *params_value;
	uint32_t param_payload_len = sizeof(struct adm_param_data_v5) +
				sizeof(struct libmfe_sound_direction_param);
	struct libmfe_sound_direction_param *sounddirection_params;
	...
	ret = adm_get_params_v2(port_id, copp_idx,
				CAPI_V2_BUNDLE_MODULE_ID,
				CAPI_V2_LIBMFE_PARAM_ID_SOUND_DIRECT,
				param_payload_len,
				params_value,
				ADM_CLIENT_ID_DEFAULT);
	if (ret) {
		pr_err("%s: get parameters failed ret:%d\n", __func__, ret);
	...
	sounddirection_params = (struct libmfe_sound_direction_param *) params_value;

	*soundDirectionData = *sounddirection_params;
	...
	struct libmfe_msg_packet_param *msgPacketData)
{
	...
	char *params_value;
	uint32_t param_payload_len = sizeof(struct adm_param_data_v5) +
				sizeof(struct libmfe_msg_packet_param);
	struct libmfe_msg_packet_param *msgPacket_params;
	...
	ret = adm_get_params_v2(port_id, copp_idx,
				CAPI_V2_BUNDLE_MODULE_ID,
				CAPI_V2_LIBMFE_PARAM_ID_MSG_PACKET,
				param_payload_len,
				params_value,
				ADM_CLIENT_ID_DEFAULT);
	if (ret) {
	...
	msgPacket_params = (struct libmfe_msg_packet_param *) params_value;

	*msgPacketData = *msgPacket_params;
...
int adm_set_libmfe_msg_packet(int port_id, int copp_idx,
	struct libmfe_msg_packet_param msgPacketData)
{
	struct adm_set_msg_packet_param msgpacket_params;
	int sz = 0;
	int ret  = 0;
	int port_idx;
	...
	sz = sizeof(struct adm_set_msg_packet_param);
	msgpacket_params.params.hdr.hdr_field =
		APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD, APR_HDR_LEN(APR_HDR_SIZE),
			      APR_PKT_VER);
	msgpacket_params.params.hdr.pkt_size = sz;
	msgpacket_params.params.hdr.src_svc = APR_SVC_ADM;
	msgpacket_params.params.hdr.src_domain = APR_DOMAIN_APPS;
	msgpacket_params.params.hdr.src_port = port_id;
	msgpacket_params.params.hdr.dest_svc = APR_SVC_ADM;
	msgpacket_params.params.hdr.dest_domain = APR_DOMAIN_ADSP;
	msgpacket_params.params.hdr.dest_port =
			atomic_read(&this_adm.copp.id[port_idx][copp_idx]);
	msgpacket_params.params.hdr.token = port_idx << 16 |
				ADM_CLIENT_ID_DEFAULT << 8 | copp_idx;
	msgpacket_params.params.hdr.opcode = ADM_CMD_SET_PP_PARAMS_V5;
	msgpacket_params.params.payload_addr_lsw = 0;
	msgpacket_params.params.payload_addr_msw = 0;
	msgpacket_params.params.mem_map_handle = 0;
	msgpacket_params.params.payload_size = sizeof(msgpacket_params) -
				sizeof(msgpacket_params.params);
	msgpacket_params.data.module_id = CAPI_V2_BUNDLE_MODULE_ID;
	msgpacket_params.data.param_id = CAPI_V2_LIBMFE_PARAM_ID_MSG_PACKET;
	msgpacket_params.data.param_size =
		msgpacket_params.params.payload_size -
		sizeof(msgpacket_params.data);
	msgpacket_params.data.reserved = 0;

	memcpy(&(msgpacket_params.msgpacket_data), &(msgPacketData),
	       sizeof(struct libmfe_msg_packet_param));
	pr_debug("%s: messageId = %d\n",
		 __func__, msgpacket_params.msgpacket_data.messageId);
	pr_debug("%s: errorCode = %d\n",
		 __func__, msgpacket_params.msgpacket_data.errorCode);

	atomic_set(&this_adm.copp.stat[port_idx][copp_idx], -1);
	ret = apr_send_pkt(this_adm.apr, (uint32_t *)&msgpacket_params);
	if (ret < 0) {
		pr_err("%s: Set params failed port = 0x%x rc %d\n",
			__func__, port_id, ret);
