...
static int libmfe_sound_direction_info(struct snd_kcontrol *kcontrol,
					   struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_BYTES;
	uinfo->count = sizeof(struct libmfe_sound_direction_param);

	return 0;
}

static int libmfe_msg_packet_info(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_BYTES;
	uinfo->count = sizeof(struct libmfe_msg_packet_param);
...
#ifdef CONFIG_AL0_CAPI_V2_LIBMFE
static int libmfe_sound_direction_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	...
	ret = msm_audio_sound_focus_derive_port_id(kcontrol,
		"Sound Direction Audio Tx ", &port_id);
	if (ret) {
		pr_err("%s: Error in deriving port id, err=%d\n",
		       __func__, ret);
	...
	ret = adm_get_libmfe_sound_direction(port_id, copp_idx, &soundDirectionData);
	if (ret) {
		pr_err("%s: Error getting Sound Direction Params, err=%d\n",
			  __func__, ret);
	...
	memcpy(ucontrol->value.bytes.data, (void *)&soundDirectionData,
...
static int libmfe_msg_packet_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	...
	ret = msm_audio_sound_focus_derive_port_id(kcontrol,
		"MicFrontEnd Msg Packet Tx ", &port_id);
	if (ret) {
		pr_err("%s: Error in deriving port id, err=%d\n",
		       __func__, ret);
	...
	ret = adm_get_libmfe_msg_packet(port_id, copp_idx, &msgPacketData);
	if (ret) {
		pr_err("%s: Error getting Msg Packet Params, err=%d\n",
			  __func__, ret);
	...
	memcpy(ucontrol->value.bytes.data, (void *)&msgPacketData,
...
static int libmfe_msg_packet_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	struct libmfe_msg_packet_param msgPacketData;
	...
	memcpy((void *)&msgPacketData, ucontrol->value.bytes.data,
		sizeof(struct libmfe_msg_packet_param));

	ret = adm_set_libmfe_msg_packet(port_id, copp_idx, msgPacketData);
	if (ret) {
		pr_err("%s: Error setting Msg Packet Params, err=%d\n",
			  __func__, ret);
...
static const struct snd_kcontrol_new msm_source_tracking_controls[] = {
...
#ifdef CONFIG_AL0_CAPI_V2_LIBMFE
	/*
	 * keep the compatibility for Sound Direction.
	 */
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READ,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "Sound Direction Audio Tx SLIMBUS_0",
		.info   = libmfe_sound_direction_info,
		.get    = libmfe_sound_direction_get,
	},
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READ,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "Sound Direction Audio Tx TERT_MI2S",
		.info   = libmfe_sound_direction_info,
		.get    = libmfe_sound_direction_get,
	},
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READ,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "Sound Direction Audio Tx QUATERNARY_MI2S",
		.info   = libmfe_sound_direction_info,
		.get    = libmfe_sound_direction_get,
	},
	/*
	 * setter/getter common IF via message packet.
	 */
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "MicFrontEnd Msg Packet Tx SLIMBUS_0",
		.info   = libmfe_msg_packet_info,
		.get    = libmfe_msg_packet_get,
		.put    = libmfe_msg_packet_set,
	},
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "MicFrontEnd Msg Packet Tx TERT_MI2S",
		.info   = libmfe_msg_packet_info,
		.get    = libmfe_msg_packet_get,
		.put    = libmfe_msg_packet_set,
	},
	{
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.iface  = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name   = "MicFrontEnd Msg Packet Tx QUATERNARY_MI2S",
		.info   = libmfe_msg_packet_info,
		.get    = libmfe_msg_packet_get,
		.put    = libmfe_msg_packet_set,
	},
